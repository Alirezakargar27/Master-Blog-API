...
def get_posts():
    """Processes get request of endpoint '/api/posts' """
...
def add_posts():
    """Processes post request of endpoint '/api/posts' """
...
def find_post_by_id(id):
    """checks if the  "ID" number exist in the database"""
    for post in POSTS:
        if post['id'] == id:
            return post
    return None
...
def delete_post(id):
    """Processes delete request of endpoint '/api/posts/<int:id>' """
...
def update_post(id):
    """Processes put request of endpoint '/api/posts/<int:id>' """
    post = find_post_by_id(id)
    error_message = {"error": f"Post with id {id} not found."}
    if post is None:
        return jsonify(error_message), 404
...
def query_search():
    """Processes get request of endpoint '/api/posts/search' """
